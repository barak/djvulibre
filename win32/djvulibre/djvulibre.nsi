;;;; Example NSIS installer script for djvulibre and djview...
...
Section -Post
  ...
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\bzz.exe"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayVersion" "${PRODUCT_VERSION}"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "URLInfoAbout" "${PRODUCT_WEB_SITE}"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "Publisher" "${PRODUCT_PUBLISHER}"
  
  WriteRegStr HKCR ".djvu" "" "DjVuFile"
  WriteRegStr HKCR ".djv" "" "DjVuFile"
  WriteRegStr HKCR "DjVuFile" "" "DjVu Document"
  WriteRegStr HKCR "DjVuFile\DefaultIcon" "" "$INSTDIR\djview.exe,1"
  WriteRegStr HKCR "DjVuFile\shell\open\command" "" '"$INSTDIR\djview.exe" "%1"'
