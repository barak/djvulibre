...
function disclaimer()
{
  echo "By using this tool you accept next disclaimer:"
...
if [ x$docformat == x'help' ]
then
    format_help; exit 0;
fi

if [ -z $docformat ] || [ ! -z ${docformat#[0-9]} ]
then
...
lurl='';
if [ ! -e $1 ]			# if 1st argument is not an existing file
then				# assume that it is URL
  lurl="$1"; 
  shift
fi
...
while [ -n "$in" ]; do
    ...
    if [ -z $lurl ]; then
	if [ ! -e $in ]; then
	    echo "File $in wasn't found. Conversion stopped"; exit 2;
	fi
