...
private:
  ...
  unsigned long ref_window, top_window;	// Actual type is Window
  int have_lock;
  void ObtainLock(void);
  void ReleaseLock(void);
  void SendCommand(const char * cmd);
public:
  unsigned long GetTopWindow(void);
  void SendPage(void);
  void SetRefWindow(unsigned long _ref_window)  // Window
  { ref_window=_ref_window; top_window=0; };
  
  RemoteNetscape(void) : displ(0), ref_window(0), top_window(0), have_lock(0) {};
  RemoteNetscape(void * _displ, unsigned long _ref_window) 
    : displ(_displ), ref_window(_ref_window), top_window(0), have_lock(0) {};
  ~RemoteNetscape(void);
};
